...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $action = $_POST['action'];
    $coin = sanitize($_POST['coin']); // e.g., bitcoin
    $amount = (float)$_POST['amount']; // Amount in NGN or Coin? Usually NGN for buy/sell

    // For this simulation, we'll store "crypto balances" in a simplified way or just mock the successful transaction
    // Realistically, we'd need a `crypto_wallets` table. Let's assume we are just logging the exchange.
    ...
    try {
        if ($action === 'buy') {
            ...
            if ($currentUser['walletBalance'] < $fee) {
                throw new Exception("Insufficient balance to cover swap fee.");
            }
            ...
            $success = "Successfully " . ($action === 'swap' ? 'swapped' : 'converted') . " $coin to $targetCoin!";
...
<div class="mx-auto min-h-screen bg-gray-50 flex flex-col pb-24">
    ...
    <div class="p-4 lg:p-8 space-y-8 flex-1">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            <?php
            $coins = [
                'bitcoin' => ['name' => 'BTC', 'color' => 'bg-orange-500', 'icon' => 'bitcoin'],
                'ethereum' => ['name' => 'ETH', 'color' => 'bg-blue-600', 'icon' => 'gem'],
                'binancecoin' => ['name' => 'BNB', 'color' => 'bg-yellow-500', 'icon' => 'coins'],
                'solana' => ['name' => 'SOL', 'color' => 'bg-purple-500', 'icon' => 'zap'],
                'tether' => ['name' => 'USDT', 'color' => 'bg-emerald-500', 'icon' => 'anchor']
            ];
            foreach ($coins as $id => $info):
                $data = $prices[$id] ?? ['ngn' => 0, 'ngn_24h_change' => 0];
                $change = $data['ngn_24h_change'] ?? 0;
            ?>
            <div class="bg-white p-5 rounded-[32px] shadow-sm border border-gray-100 space-y-3">
                <div class="flex items-center justify-between">
                    <div class="w-8 h-8 <?php echo $info['color']; ?> rounded-xl flex items-center justify-center text-white shadow-lg">
                        <i data-lucide="<?php echo $info['icon']; ?>" class="w-4 h-4"></i>
                    </div>
                    <span class="text-[10px] font-black <?php echo $change >= 0 ? 'text-green-500' : 'text-red-500'; ?>">
                ...
                <div>
                    ...
                    <div class="text-sm font-black text-gray-900">₦<?php echo number_format($data['ngn'], 2); ?></div>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
                    ...
                    <form method="POST" id="cryptoForm" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div id="fromGroup">
                                ...
                                <select name="coin" id="coinSelect" onchange="updateExchange()" class="w-full p-5 bg-gray-50 rounded-2xl font-black text-lg border-2 border-transparent focus:border-billpay-green outline-none transition-all">
                                    <?php foreach ($coins as $id => $info): ?>
                                    <option value="<?php echo $id; ?>"><?php echo $info['name']; ?> - <?php echo ucfirst($id); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div id="toGroup" class="hidden">
                                ...
                                <select name="targetCoin" id="targetCoinSelect" class="w-full p-5 bg-gray-50 rounded-2xl font-black text-lg border-2 border-transparent focus:border-billpay-green outline-none transition-all">
                                    <?php foreach ($coins as $id => $info): ?>
                                    <option value="<?php echo $id; ?>" <?php echo $id === 'tether' ? 'selected' : ''; ?>><?php echo $info['name']; ?> - <?php echo ucfirst($id); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
            ...
            <div class="space-y-6">
                <div class="bg-gradient-to-br from-gray-900 to-black p-8 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
                    <div class="relative z-10">
                        <div class="text-[10px] font-black uppercase tracking-widest opacity-50 mb-2">Estimated Balance</div>
                        <div class="text-3xl font-black tracking-tighter">₦<?php echo number_format($currentUser['walletBalance'], 2); ?></div>
                        <div class="mt-8 pt-8 border-t border-white/10 space-y-4">
                            <div class="flex justify-between items-center">
                                <span class="text-[10px] font-black uppercase opacity-40">Portfolio Value</span>
                                <span class="text-xs font-black text-billpay-green">+₦12,450.00</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-[10px] font-black uppercase opacity-40">Active Assets</span>
                                <span class="text-xs font-black">5 Coins</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm">
                    <h3 class="text-xs font-black uppercase tracking-widest mb-6">Recent Activity</h3>
                    <div class="space-y-6">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 bg-green-50 text-green-500 rounded-xl flex items-center justify-center"><i data-lucide="arrow-down-left" class="w-5 h-5"></i></div>
                            <div>
                                <div class="text-[10px] font-black text-gray-900 uppercase">Received BTC</div>
                                <div class="text-[9px] font-bold text-gray-400 uppercase">2 hours ago</div>
                            </div>
                        </div>
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 bg-blue-50 text-blue-500 rounded-xl flex items-center justify-center"><i data-lucide="repeat" class="w-5 h-5"></i></div>
                            <div>
                                <div class="text-[10px] font-black text-gray-900 uppercase">Swapped ETH/USDT</div>
                                <div class="text-[9px] font-bold text-gray-400 uppercase">Yesterday</div>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function setMode(mode) {
        ...
        const btns = ['Buy', 'Sell', 'Swap', 'Withdraw'];
        btns.forEach(b => {
        ...
        if (mode === 'buy') {
            ...
            submitBtn.innerText = 'Sell to Wallet';
            addressGroup.classList.add('hidden');
            toGroup.classList.add('hidden');
        } else if (mode === 'swap') {
            submitBtn.innerText = 'Swap Assets';
            addressGroup.classList.add('hidden');
            toGroup.classList.remove('hidden');
        } else if (mode === 'withdraw') {
            submitBtn.innerText = 'Withdraw to External Wallet';
