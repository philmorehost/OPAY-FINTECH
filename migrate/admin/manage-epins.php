...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'process_epin') {
        ...
        if ($epin) {
            ...
            $success = "EPIN [{$pin}] processed for phone [{$phone}]. Data plan: {$epin['planName']}";
        } else {
            ...
            <div class="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100">
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="pin" placeholder="XXXX-XXXX-XXXX" required class="w-full p-4 bg-gray-50 rounded-2xl font-black text-lg outline-none border border-transparent focus:border-billpay-green">
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase ml-1">Customer Phone</label>
                    ...
                    <button type="submit" class="w-full py-5 bg-gray-900 text-white rounded-2xl font-black uppercase text-xs shadow-xl">Process & Mark Used</button>
                </form>
            </div>
        </div>
