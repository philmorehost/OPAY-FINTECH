    ...
    <div class="bg-white p-6 rounded-[40px] shadow-sm space-y-8 border border-gray-100">
      <form method="POST">
        ...
        <div class="grid grid-cols-4 gap-3"><?php foreach ($cableProviders as $p): ?>
          <button type="button" onclick="document.getElementById('providerId').value='<?php echo h($p['id']); ?>'; this.form.submit();" class="flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all <?php echo ($providerId ?? '') === $p['id'] ? 'border-vtu-green bg-green-50' : 'border-transparent bg-gray-50 opacity-60'; ?>">
        ...
        <div class="mt-6"><label class="block text-[10px] font-black text-gray-400 mb-2 uppercase tracking-widest ml-1">IUC Number</label><div class="flex gap-2"><input type="tel" name="smartcardNumber" value="<?php echo h($smartcardNumber ?? ''); ?>" placeholder="Enter number" class="flex-1 p-4 bg-gray-50 text-gray-900 border-2 border-transparent focus:border-vtu-green outline-none rounded-2xl font-black text-lg" /><button type="submit" class="p-4 bg-gray-900 text-white rounded-2xl"><i data-lucide="search" size="20"></i></button></div></div>
      </form>
      <?php if ($customerName): ?>
        <form method="POST" class="space-y-6 animate-fade-in"><input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>"><input type="hidden" name="action" value="pay"><input type="hidden" name="providerId" value="<?php echo h($providerId); ?>"><input type="hidden" name="smartcardNumber" value="<?php echo h($smartcardNumber); ?>"><input type="hidden" name="amount" value="<?php echo h($renewalAmount); ?>">
          <div class="p-4 bg-green-50 rounded-2xl border border-green-100 flex items-center gap-3"><i data-lucide="user" class="text-vtu-green"></i><span class="text-xs font-black"><?php echo h($customerName); ?></span></div>
          <button type="submit" class="w-full bg-vtu-green text-white font-black py-5 rounded-[24px] shadow-xl">PAY <?php echo format_currency($renewalAmount); ?></button>
        </form><?php endif; ?>
    </div>
  </div>
