...
<div class="max-w-md mx-auto min-h-screen bg-gray-50 flex flex-col animate-fade-in">
  ...
  <div class="p-4 flex-1">
    ...
    <form method="POST" class="bg-white p-6 rounded-3xl shadow-sm space-y-8 border border-gray-100">
      ...
      <div class="flex bg-gray-100 p-1.5 rounded-2xl"><button type="button" onclick="document.getElementById('isBulkInput').value='false'" class="flex-1 py-3.5 rounded-xl text-[11px] font-black uppercase bg-white shadow-lg text-opay-green">Single</button><button type="button" onclick="document.getElementById('isBulkInput').value='true'" class="flex-1 py-3.5 rounded-xl text-[11px] font-black uppercase text-gray-400">Bulk</button></div>
      <div id="singleFields"><label class="block text-[11px] font-black text-gray-400 mb-2 uppercase tracking-widest">Phone Number</label><input type="tel" name="phoneNumber" placeholder="08012345678" class="w-full p-5 bg-gray-50 text-gray-900 border-2 border-transparent focus:border-opay-green outline-none rounded-2xl font-black text-xl" /></div>
      <div id="amountField"><label class="block text-[11px] font-black text-gray-400 mb-2 uppercase tracking-widest">Amount</label><input type="number" name="amount" placeholder="Min 50" class="w-full p-5 bg-gray-50 text-gray-900 border-2 border-transparent focus:border-opay-green outline-none rounded-2xl font-black text-xl" /></div>
      <button type="submit" class="w-full bg-opay-green text-white font-black py-5 rounded-2xl shadow-xl">Confirm & Buy</button>
    </form>
  </div>
  <?php if ($statusDetails): ?><div class="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-6"><div class="bg-white w-full max-w-sm rounded-[40px] overflow-hidden p-8 text-center"><h3 class="text-xl font-black uppercase text-opay-green">Successful</h3><div class="text-3xl font-black mt-2"><?php echo format_currency($statusDetails['amount']); ?></div><button onclick="window.location.href='dashboard'" class="w-full mt-8 py-5 rounded-3xl bg-gray-900 text-white font-black">DONE</button></div></div><?php endif; ?>
</div>
